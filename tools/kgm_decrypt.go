package tools

// This is a tool that can decrypt KGMusic and Vprite2 encrypted files.

import (
	"encoding/binary"
	"fmt"
	"os"
	"path/filepath"
	"strings"
)

var (
	VprHeader   = [16]byte{0x05, 0x28, 0xBC, 0x96, 0xE9, 0xE4, 0x5A, 0x43, 0x91, 0xAA, 0xBD, 0xD0, 0x7A, 0xF5, 0x36, 0x31}
	KgmHeader   = [16]byte{0x7C, 0xD5, 0x32, 0xEB, 0x86, 0x02, 0x7F, 0x4B, 0xA8, 0xAF, 0xA6, 0x8E, 0x0F, 0xFF, 0x99, 0x14}
	VprMaskDiff = [17]byte{0x25, 0xDF, 0xE8, 0xA6, 0x75, 0x1E, 0x75, 0x0E, 0x2F, 0x80, 0xF3, 0x2D, 0xB8, 0xB6, 0xE3, 0x11, 0x00}
	Table1      = [272]byte{
		}
	Table2 = [272]byte{
		}
	MaskV2PreDef = [272]byte{
		}
)

func getMask(pos int) byte {
	offset := pos >> 4
	value := byte(0)
	for offset >= 0x11 {
		value ^= Table1[offset%272]
		offset >>= 4
		value ^= Table2[offset%272]
		offset >>= 4
	}

	return MaskV2PreDef[pos%272] ^ value
}

func decrypt(data []byte, isVpr bool) []byte {
	headerLen := binary.LittleEndian.Uint32(data[0x10:0x14])
	key := make([]byte, 17)
	copy(key, data[0x1C:0x2C])
	key[16] = 0
	data = data[headerLen:]

	result := make([]byte, len(data))

	for i := range data {
		med8 := key[i%17] ^ data[i]
		med8 ^= (med8 & 0xf) << 4

		msk8 := getMask(i)
		msk8 ^= (msk8 & 0xf) << 4
		result[i] = med8 ^ msk8

		if isVpr {
			result[i] ^= VprMaskDiff[i%17]
		}
	}

	return result
}

func start_kgm_decrypt() {
	inputDir := "./resources/kgm"
	outputDir := "./resources/mp3/"
	files, err := os.ReadDir(inputDir)
	if err != nil {
		fmt.Println("Error reading directory:", err)
		return
	}

	for _, file := range files {
		fileName := file.Name()
		if strings.HasSuffix(fileName, ".kgm") {
			inputPath := filepath.Join(inputDir, fileName)
			outputPath := outputDir + strings.TrimSuffix(fileName, ".kgm") + ".mp3"

			data, err := os.ReadFile(inputPath)
			if err != nil {
				fmt.Println("Error reading file:", inputPath, err)
				continue
			}

			isVpr := false // Modify this based on your file type detection
			decryptedData := decrypt(data, isVpr)

			err = os.WriteFile(outputPath, decryptedData, 0644)
			if err != nil {
				fmt.Println("Error writing file:", outputPath, err)
				continue
			}

			fmt.Println("Decrypted:", inputPath, "->", outputPath)
		}
	}
}
